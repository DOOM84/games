  ...
  <main  class="center pb-2 adminHome" >
    ...
    <AdminModalWrap @closeDlg="closeModal" mWidth="1000px" origWidth="100%" :showDlg="showDlg">
      ...
      <div class="form-group left">
        ...
        <input type="text" v-model.trim="developerToUpdate.name" class="form-control " id="name">
      </div>

      <div class="right mt-2 mr-2 admin-opts">
      <div>
        <label for="status" class="admin-status">Опубликовано</label>
        <input type="checkbox" v-model="developerToUpdate.status" id="status">
      </div>
...
const {data, error} = await useAsyncData('adminDevelopers', () => $fetch('/api/admin/developers'));
...
const developerToUpdate = ref({status: false});
...
function closeModal() {
  showDlg.value = false;
  mode.value = null;
  developerToUpdate.value = {status: false}
}

function updateItem(developer) {
  mode.value = 'edit';
  developerToUpdate.value = {...developer}
  showDlg.value = true;
}
...
async function storeItem() {
  ...
  formData.append('data', JSON.stringify(developerToUpdate.value))

  try {
    $showToast('Обработка...', 'info', 2000);

    if (mode.value === 'edit') {
      const {result} = await $fetch('/api/admin/developers/edit', {
        method: 'PUT',
        body: formData,
      })
      const ind = data.value.developers.findIndex(item => item.id === result.id);
      data.value.developers[ind] = result;
    }

    if (mode.value === 'add') {
      const {result} = await $fetch('/api/admin/developers/add', {
        method: 'POST',
        body: formData,
      })
      data.value.developers.unshift(result);
    }
...
async function removeItem(dbId) {
  if (confirm('Are you sure?')) {
    try {
      ...
      const {id} = await $fetch('/api/admin/developers/remove', {
        method: 'DELETE',
        body: formData,
      })

      data.value.developers.splice(data.value.developers.findIndex(item => item.id === id), 1);
