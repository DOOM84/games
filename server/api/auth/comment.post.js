    ...
    try {
        ...
        if(!parentId){
            ...
            if(!allComments || Object.keys(allComments).length === 0){
                ...
                await db.collection('comments').doc(id).set({comments: [newComment]})

            }else{

                const {comments} = allComments;
                comments.push(newComment);
                await db.collection('comments').doc(id).update({comments})
            ...
            const allC = (await db.collection('comments').doc(id).get()).data();
            ...
            await db.collection('comments').doc(id).set(allC);

            const childComments = (await db.collection('comments').doc(id).get()).data()[parentId]
